...
def declare_logic():
    ...
    # Calculate the total value for each inventory record.
    Rule.formula(derive=Inventory.total_value, as_expression=lambda row: row.quantity * row.cost)

    # Calculate the total value for store inventory.
    Rule.formula(derive=StoreInventory.total_value, as_expression=lambda row: row.quantity * row.inventory.cost)
